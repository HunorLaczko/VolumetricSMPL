"""AMASS SMPL-X sequence scanning.

Reads AMASS `*_stageii.npz` files and produces flat arrays of SMPL-X parameters, one row
per selected body.
"""
...
class PoseCache:
    ...
    params: dict[str, np.ndarray]  # each (N, D)
    seq_names: list[str]
    frame_ids: list[int]
...
def scan(data_root: str, subsets: list[str], stride, num_betas: int = NUM_BETAS,
    ...
    params = {k: np.concatenate(v, axis=0) for k, v in acc.items()}
